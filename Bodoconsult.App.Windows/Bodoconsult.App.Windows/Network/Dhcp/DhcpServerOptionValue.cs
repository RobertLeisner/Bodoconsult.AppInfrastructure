...
public class DhcpServerOptionValue : IDhcpServerOptionValue
{
    ...
    private DhcpServerOption _option;
    /// <summary>
    /// The associated Option
    /// </summary>
    public IDhcpServerOption Option => _option ??= GetOption();
    ...
    private static DhcpServerOptionValue FromNative(DhcpServer server, in DhcpOptionValue native, string className, string vendorName)
    {
    ...
    {
        var scopeInfo = new DhcpOptionScopeInfoManagedReserved(reservation.Scope.Address.ToNativeAsNetwork(), reservation.Address.ToNativeAsNetwork());
    ...
    {
        var scopeInfo = new DhcpOptionScopeInfoManagedSubnet(address.ToNativeAsNetwork());
    ...
    {
        var scopeInfo = new DhcpOptionScopeInfoManagedSubnet(scopeAddress.ToNativeAsNetwork());
    ...
    {
        var reservationInfo = new DhcpOptionScopeInfoManagedReserved(scopeAddress.ToNativeAsNetwork(), reservationAddress.ToNativeAsNetwork());
    ...
    {
        var scopeInfo = new DhcpOptionScopeInfoManagedGlobal(IntPtr.Zero);
    ...
    {
        var scopeInfo = new DhcpOptionScopeInfoManagedSubnet(scope.Address.ToNativeAsNetwork());
    ...
    private static IEnumerable<DhcpServerOptionValue> GetAllOptionValues(DhcpServer server, IntPtr scopeInfo)
    {
        ...
        try
        {
            using (var values = valuesPtr.MarshalToStructure<DhcpAllOptionValues>())
            {
    ...
    private static IEnumerable<DhcpServerOptionValue> EnumOptionValuesV0(DhcpServer server, IntPtr scopeInfo)
    {
        ...
        try
        {
            using (var valueArray = valueArrayPtr.MarshalToStructure<DhcpOptionValueArray>())
            {
    ...
    private static IEnumerable<DhcpServerOptionValue> EnumOptionValuesV5(DhcpServer server, IntPtr scopeInfo, string className, string vendorName)
    {
        ...
        var result = Api.DhcpEnumOptionValuesV5(serverIpAddress: server.Address,
            flags: (vendorName == null) ? 0 : Constants.DhcpFlagsOptionIsVendor,
            className: className,
            vendorName: vendorName,
            scopeInfo: scopeInfo,
            resumeHandle: ref resumeHandle,
    ...
    private static DhcpServerOptionValue GetOptionValueV0(DhcpServer server, IntPtr scopeInfo, int optionId)
    {
        ...
        try
        {
            using (var value = valuePtr.MarshalToStructure<DhcpOptionValue>())
            {
