...
public class DhcpServerFailoverRelationship : IDhcpServerFailoverRelationship
{
    ...
    private readonly List<DhcpServerIpAddress> _scopeAddresses;
    public IEnumerable<IDhcpServerScope> Scopes
    {
        get
        {
            // shortcut
            if (_scopeAddresses?.Count == 0)
                yield break;

            foreach (var scopeAddress in _scopeAddresses)
    ...
    )
    {
        ...
        _scopeAddresses = scopeAddresses;
    ...
    internal static DhcpServerFailoverRelationship GetFailoverRelationship(DhcpServer server, string relationshipName)
    {
        ...
        try
        {
            using (var relationship = relationshipPtr.MarshalToStructure<DhcpFailoverRelationship>())
            {
    ...
    internal static IEnumerable<DhcpServerFailoverRelationship> GetFailoverRelationships(DhcpServer server)
    {
        ...
        try
        {
            using (var relationships = relationshipsPtr.MarshalToStructure<DhcpFailoverRelationshipArray>())
            {
    ...
    internal static void AddScopeToFailoverRelationship(DhcpServerFailoverRelationship relationship, DhcpServerScope scope)
    {
        ...
        using (var relationshipNative = new DhcpFailoverRelationshipManaged(relationshipName: relationship.Name,
                   scopes: new DhcpIpArrayManaged(scope.Address.ToNativeAsNetwork())))
        {
        ...
        // update local cache
        relationship._scopeAddresses.Add(scope.Address);
    ...
    internal static DhcpServerFailoverRelationship CreateFailoverRelationship(DhcpServerScope scope, DhcpServer partnerServer, string name, string sharedSecret, DhcpServerFailoverMode mode, byte modePercentage, TimeSpan maximumClientLeadTime, TimeSpan? stateSwitchInterval)
    {
        ...
        if (!scope.Server.IsCompatible(DhcpServerVersions.Windows2012) || !partnerServer.IsCompatible(DhcpServerVersions.Windows2012))
            throw new DhcpServerException(nameof(Api.DhcpV4FailoverCreateRelationship), DhcpErrors.ErrorInvalidParameter, "Windows Server 2012 is required to establish a failover relationship");
        ...
        if (existingRelationship != null)
            throw new DhcpServerException(nameof(Api.DhcpV4FailoverCreateRelationship), DhcpErrors.FoScopeAlreadyInRelationship);

        // create/replicate
        var partnerScope = CreateOrReplicatePartnerScope(partnerServer, scope);

        // define relationship
        using (var primaryRelationship = new DhcpFailoverRelationshipManaged(primaryServer: primaryServerAddress.ToNativeAsNetwork(),
                   secondaryServer: partnerServer.Address.ToNativeAsNetwork(),
                   mode: (DhcpFailoverMode)mode,
                   serverType: DhcpFailoverServer.PrimaryServer,
                   state: FsmState.NoState,
                   prevState: FsmState.NoState,
    ...
    private static DhcpServerScope CreateOrReplicatePartnerScope(DhcpServer partnerServer, DhcpServerScope sourceScope)
    {
        ...
        catch (DhcpServerException ex) when (ex.ApiErrorNative == DhcpErrors.SubnetNotPresent)
        {
    ...
    internal static void DeleteFailoverRelationship(DhcpServerFailoverRelationship relationship)
    {
        ...
        if (relationship._scopeAddresses.Count > 0)
    ...
    private static DhcpServerFailoverRelationship FromNative(DhcpServer server, in DhcpFailoverRelationship native)
    {
    ...
    private static DhcpServerFailoverRelationship FromNative(DhcpServer server, in DhcpFailoverRelationshipManaged native)
    {
        return new DhcpServerFailoverRelationship(server: server,
            name: native.RelationshipName,
            mode: (DhcpServerFailoverMode)native.Mode,
