...
public class ColumnChart<T> : BaseChart<T> where T : IChartItemData
{
    ...
    public override void Formatting()
    {
        ...
        Chart.Grid.YAxisStyle.MajorLineStyle.Width = 0;

        Chart.Grid.XAxisStyle.MajorLineStyle.Width = 1;
        Chart.Grid.XAxisStyle.MajorLineStyle.Color = style.GridLineColor.ToScottPlotColor();
        Chart.Grid.XAxisStyle.MajorLineStyle.Pattern = LinePattern.Dashed;
        ...
        Chart.RenderManager.RenderStarting += (_, _) =>
        {
            ...
            for (var i = 0; i < Ticks.Length; i++)
            {
                ...
                label = value.ToString(formatY, CultureInfo.InvariantCulture);
                Ticks[i] = new Tick(Ticks[i].Position, label);
            }

            Chart.Axes.Bottom.TickLabelStyle.IsVisible = false;
            Chart.Axes.Bottom.MajorTickStyle.Length = 0;
        };
    }
