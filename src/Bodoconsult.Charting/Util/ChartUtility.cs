...
public class ChartUtility
{
    ...
    /// <param name="columns">List with columns or null</param>
    /// <param name="chartData">Current <see cref="IChartData"/> instance</param>
    public static void DataTableToPieChartItemData(DataTable dataTable, string columns, IChartData chartData)
    {
        ...
        for (var index = 0; index < dataTable.Rows.Count; index++)
        {
            ...
            if (label.Equals("Gesamt") || label.Equals("Total"))
            {
                continue;
            }
    ...
    /// <param name="columns">List with columns or null</param>
    /// <param name="chartData">Current <see cref="IChartData"/> instance</param>
    public static void DataTableToPointChartItemData(DataTable dataTable, string columns, IChartData chartData)
    {
        ...
        for (var index = 0; index < dataTable.Rows.Count; index++)
        {
            ...
            var item = new PointChartItemData
            {
                ...
                Color = TypoColor.FromUInt32(Convert.ToUInt32(row[colColor].ToString())),
            };
    ...
    /// <param name="columns">List with columns or null</param>
    /// <param name="chartData">Current <see cref="IChartData"/> instance</param>
    public static void DataTableToChartItemData(DataTable dataTable, string columns, IChartData chartData)
    {
        int length;
        int[] columnArray;
        int colLabel;
        ...
        for (var index = 0; index < dataTable.Rows.Count; index++)
        {
            ...
            var data = row.ItemArray[colLabel];
            ...
            else
            {
                if (isDate)
                {
                    label = ((DateTime)data).ToOADate();
                }
                else
                {
                    var success = double.TryParse(data.ToString(), out var x);
        ...
        {
            var columnName = dataTable.Columns[columnArray[i]].ColumnName;
            chartData.LabelsForSeries.Add(columnName);
            chartData.PropertiesToUseForChart.Add(columnName);
        }
    }
    ...
    /// </summary>
    /// <param name="value">Double value as string</param>
